...
buildList start count end = repeat count [end]
    where
        repeat 0 acc = acc
        repeat n acc = repeat (n - 1) (start : acc)
...
sums i = sums' 1 0
   where
    sums' n accSum
        | n > i = []
        | otherwise = (accSum + n) : sums' (n + 1) (accSum + n)
...
indexDefault (h:t) i def
    | i < 0 = def
    | i == 0 = h
    | otherwise = indexDefault t (i - 1) def
...
sumsOf xs = sum' xs 0
    where
        sum' [] acc = []
        sum' (x:xs) acc = let newAcc = acc + x in newAcc : sum' xs newAcc
...
mymaximum bigger initial (x:xs) =
    if bigger initial x then mymaximum bigger initial xs else mymaximum bigger x xs
