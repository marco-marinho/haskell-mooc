...
isOrdered (Node v left right) = 
  case (left, right) of
    (Node lv _ _, Node rv _ _) -> lv < v && rv > v && isOrdered left && isOrdered right
    (Empty, Node rv _ _) -> rv > v && isOrdered right
    (Node lv _ _, Empty) -> lv < v && isOrdered left
    (Empty, Empty) -> True
...
set [] val tree = case tree of
  Empty -> Empty
  Node _ left right -> Node val left right
set (x:xs) val tree = 
  case x of 
    StepL -> case tree of
      Empty -> Empty
      Node v left right -> Node v (set xs val left) right
    StepR -> case tree of
      Empty -> Empty
      Node v left right -> Node v left (set xs val right)
