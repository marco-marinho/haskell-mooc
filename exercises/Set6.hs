...
simplify (RationalNumber a b) = if common == 1 
  then RationalNumber a b 
  else simplify (RationalNumber (a `div` common) (b `div` common))
    where
      common = gcd a b
...
instance Num RationalNumber where
  ...
  abs (RationalNumber a b) = RationalNumber (if a < 0 then -a else a) (if b < 1 then -b else b)
