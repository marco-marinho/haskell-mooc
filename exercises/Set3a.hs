...
import Data.ByteString (reverse, takeWhile)
import Distribution.Compat.Lens (toDListOf)
...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b =
    case measure a `compare` measure b of
        GT -> a
        LT -> b
        _ -> a
...
firstHalf x = take (halfLen x) x
    where halfLen x = (length x + 1) `div` 2

palindrome :: String -> Bool
palindrome x = Data.List.reverse x == x
...
capitalize x = unwords $ map captalizeFirst $ words x
    where captalizeFirst (x:xs) = toUpper x : xs
...
powers :: Int -> Int -> [Int]
powers k max = Data.List.takeWhile (<= max) [k ^ n | n <- [0..]]
...
joinToLength n lst = [s1 ++ s2 | s1 <- lst, s2 <- lst, length (s1 ++ s2) == n]
...
interpreter commands = interpret [] commands 0 0
    where
        interpret acc [] _ _ = acc
        interpret acc (h:t) x y = case h of
            "up" -> interpret acc t x (y + 1)
            "down" -> interpret acc t x (y - 1)
            "left" -> interpret acc t (x - 1) y
            "right" -> interpret acc t (x + 1) y
            "printX" -> interpret (acc ++ [show x]) t x y
            "printY" -> interpret (acc ++ [show y]) t x y
            _ -> interpret acc t x y
