...
instance Functor TwoList where
  ...
  fmap f (TwoNode val1 val2 rest) = TwoNode (f val1) (f val2) (fmap f rest)
...
count a = foldr (\y acc -> if y == a then acc + 1 else acc) 0
...
inBoth f g = [x | x <- toList f, x `elem` toList g]
...
instance Foldable List where
  ...
  foldr f initialValue (LNode val rest) = f val (foldr f initialValue rest)
...
instance Foldable TwoList where
  ...
  foldr f initialValue (TwoNode val1 val2 rest) = f val1 (f val2 (foldr f initialValue rest))
