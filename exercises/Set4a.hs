...
rangeOf lst = last sortLst - head sortLst
    where
        sortLst = sort lst
    ...
    where
        ...
        go a (b:t) =
            case compare (length a) (length b) of
                GT -> go a t
                LT -> go b t
                EQ -> if head a < head b then go a t else go b t
...
incrementKey _ [] = []
incrementKey t ((k, v):xs) = if k == t
    then (k, v + 1) : incrementKey t xs
    else (k, v) : incrementKey t xs
...
freqs = foldr (Map.alter (\case Nothing -> Just 1; Just n -> Just (n+1))) Map.empty
...
transfer from to amount bank =
    case (Map.lookup from bank, Map.lookup to bank) of
        (Nothing, _) -> bank
        (_, Nothing) -> bank
        (Just f, Just t) -> if (amount < 0) || (f < amount) then bank else Map.insert to (t + amount) (Map.insert from (f - amount) bank)
...
maxIndex arr = fst (go (Data.Array.assocs arr))
    where 
        go (x:xs) = foldr (\(i, v) (j, max) -> if v > max then (i, v) else (j, max)) x xs
