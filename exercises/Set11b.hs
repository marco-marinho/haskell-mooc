...
appendPure :: String -> String -> String
appendPure a b = b ++ a

appendAll :: IORef String -> [String] -> IO ()
appendAll acc [] = return ()
appendAll acc (s:ss) = do
    modifyIORef acc (appendPure s)
    appendAll acc ss
...
hSelectLines' _ [] _ = return []
hSelectLines' handle (x:xs) curr = do
    v <- hGetLine handle
    if x == curr
        then do
            rest <- hSelectLines' handle xs (curr + 1)
            return (v : rest)
        else do
            hSelectLines' handle (x:xs) (curr + 1)

hSelectLines :: Handle -> [Int] -> IO [String]
hSelectLines h l = hSelectLines' h (sort l) 1
...
interact' f state = do 
    line <- getLine
    let (obool, ostr , ost) = f (line, state)
    putStrLn ostr
    if obool
        then interact' f ost
        else return ost
