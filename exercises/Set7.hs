...
add m (Set xs) = Set (go m xs)
  where go m [] = [m]
        go m (x:xs)
          | m == x  = x:xs  -- already in the set, do nothing
          | m < x   = m:x:xs -- insert before x
          | otherwise = x : go m xs -- continue searching
...
data State = Start | Error | Finished | HasEggs | HasFlour | HasSugar | HasEverything | Mixed
  deriving (Eq,Show)

step :: State -> Event -> State
step s e = case (e, s) of
  (AddEggs, Start) -> HasEggs
  (AddFlour, HasEggs) -> HasFlour
  (AddSugar, HasEggs) -> HasSugar
  (AddFlour, HasSugar) -> HasEverything
  (AddSugar, HasFlour) -> HasEverything
  (Mix, HasEverything) -> Mixed
  (Bake, Mixed) -> Finished
  (_, Finished) -> Finished
  _ -> Error
...
average (h :| t)= (h + sum t) / fromIntegral (1 + length t)
...
reverseNonEmpty (h :| t) = head reversed :| tail reversed
  where reversed = reverse (h:t)
...
passwordAllowed pss req = case req of
  MinimumLength n -> length pss >= n
  ContainsSome chars -> any (`elem` pss) chars
  DoesNotContain chars -> all (`notElem` pss) chars
  And req1 req2 -> passwordAllowed pss req1 && passwordAllowed pss req2
  Or req1 req2 -> passwordAllowed pss req1 || passwordAllowed pss req2
...
evaluate ex = case ex of
  Literal i -> fromIntegral i
...
render ex = case ex of
  Literal i -> show i
  Operation op x y -> "(" ++ render x ++ op ++ render y ++ ")"
